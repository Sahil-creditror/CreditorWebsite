...
export default function InfographicTable({
  ...
  return (
    <section
      className={`${className} relative w-full px-4 py-20 overflow-hidden`}
    >
      ...
      <div className="relative mx-auto max-w-6xl w-full">
        ...
        <header className="mb-12 text-left relative z-10">
          <motion.div
            ...
            className="inline-flex items-center justify-start rounded-full bg-blue-100/70 dark:bg-blue-900/40 px-4 py-2 mb-4 backdrop-blur-sm"
          >
          ...
          <h2 className="text-5xl font-extrabold tracking-tight text-blue-600 dark:text-blue-600">
        ...
        {/* Column headers */}
        <div className="hidden md:grid grid-cols-[80px_1fr_160px_200px] gap-6 mb-4 px-6 relative z-10">
        ...
        <div className="space-y-4 relative z-10">
          {items.map((item, i) => (
            <motion.div
              ...
              className="grid grid-cols-1 md:grid-cols-[80px_1fr_160px_200px] items-center gap-4 md:gap-6 rounded-xl border border-gray-200/20 dark:border-gray-700/40 bg-white/70 dark:bg-gray-800/60 backdrop-blur-sm p-5 shadow-sm hover:shadow-lg transition-all cursor-pointer group"
            ...
            >
              ...
              <div className="flex justify-center md:justify-start">
                <div
                  className={`flex h-12 w-12 items-center justify-center rounded-lg bg-gradient-to-br ${item.accent} text-white shadow-sm group-hover:scale-105 transition-transform`}
                >
                  {item.icon}
                </div>
              </div>
              ...
              {/* Income */}
              <div className="flex justify-center md:justify-start">
                <div className="inline-flex items-center rounded-full bg-blue-50/70 dark:bg-blue-900/40 px-3 py-1.5 text-sm font-medium text-blue-700 dark:text-blue-300 group-hover:bg-blue-100/80 dark:group-hover:bg-blue-800/50 transition-colors backdrop-blur-sm">
                  {item.income}
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2 justify-center md:justify-start">
                {item.support.map((s, idx) => (
                  <span
                    key={idx}
                    className="inline-flex items-center rounded-md bg-gray-50/80 dark:bg-gray-700/50 px-2.5 py-1 text-xs font-medium text-gray-700 dark:text-gray-300 group-hover:bg-gray-100/90 dark:group-hover:bg-gray-600/50 transition-colors backdrop-blur-sm"
                  >
                    {s}
                  </span>
                ))}
        ...
        >
          <motion.button
            ...
            onClick={onCta}
            className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-3 text-base font-semibold text-white shadow-md hover:shadow-lg transition-all"
          >
            <span>{ctaLabel}</span>
