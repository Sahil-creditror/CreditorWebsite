...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!mod) return {};
  return {
    title: `${mod.title} | Financial Freedom | Creditor Academy`,
    description: `${mod.title} — ${mod.totalLessons} lessons in the Financial Freedom program.`,
    alternates: { canonical: `${siteUrl}/financial-freedom/${slug}` },
  };
}

export default async function FinancialFreedomModulePage({ params }: Props) {
  const { module: slug } = await params;
  const mod = getModuleBySlug(course, slug);
  if (!mod) notFound();
