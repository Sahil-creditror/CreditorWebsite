...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${mod.title} — ${trackLabel} | Operate Private | Creditor Academy`,
    description: `${trackLabel} lessons for ${mod.title} in the Operate Private program.`,
    alternates: { canonical: `${siteUrl}/operate-private/${moduleSlug}/${track}` },
  };
}

export default async function OperatePrivateModuleTrackPage({ params }: Props) {
  const { module: moduleSlug, track } = await params;
  const mod = getModuleBySlug(course, moduleSlug);
